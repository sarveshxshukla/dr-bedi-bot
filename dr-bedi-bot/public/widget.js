/* Dr. Rajeev Bedi OPD Chat Widget — paste ONE script tag on the site:
...
(function () {
  ...
  // Editable greeting (set in staff panel -> Settings). Falls back to default if unset.
  var CFG_GREETING = "";
  ...
  // --- Indian Phone Validation (Mobiles) ---
  ...
  var C = { 
    ...
    launch: "#FF6B35", launchDeep: "#E85724",   // Launcher bubble orange
  ...
  var css = "" +
    ...
    "#bedi-btn{position:fixed;right:18px;bottom:96px;width:60px;height:60px;border-radius:50%;border:none;cursor:pointer;z-index:2147483000;box-shadow:0 10px 26px rgba(255,107,53,.45);background:linear-gradient(135deg," + C.launch + "," + C.launchDeep + ");animation:bedinudge 6s ease-in-out infinite;display:flex;align-items:center;justify-content:center;transition:transform .2s}" +
    "#bedi-btn:hover{filter:brightness(1.1)}"+
    "#bedi-btn::after{content:'';position:absolute;inset:0;border-radius:50%;pointer-events:none;animation:bediring 2.6s cubic-bezier(.25,.6,.35,1) infinite}" +
    "@keyframes bediring{0%{box-shadow:0 0 0 0 rgba(255,107,53,.55)}70%{box-shadow:0 0 0 18px rgba(255,107,53,0)}100%{box-shadow:0 0 0 0 rgba(255,107,53,0)}}" +
    "@keyframes bedinudge{0%,86%,100%{transform:translateY(0)}90%{transform:translateY(-8px)}94%{transform:translateY(-3px)}}" +
    "@media(prefers-reduced-motion:reduce){#bedi-btn,#bedi-btn::after{animation:none}}" +
    "#bedi-pop{position:fixed;right:18px;bottom:164px;max-width:235px;background:#fff;color:" + C.ink + ";border:1px solid " + C.line + ";padding:12px 30px 12px 14px;border-radius:16px 16px 4px 16px;box-shadow:0 12px 34px rgba(10,77,74,.18);z-index:2147482999;cursor:pointer;font-size:14px;line-height:1.45;display:none}" +
    ...
    "#bedi-panel{position:fixed;right:18px;bottom:86px;width:380px;max-width:calc(100vw - 28px);height:620px;max-height:calc(100vh - 106px);background:#fff;border:1px solid #eef0f2;border-radius:20px;overflow:hidden;display:none;flex-direction:column;z-index:2147483000;box-shadow:0 24px 60px rgba(10,77,74,.22)}" +
    "#bedi-panel.fit{height:auto!important;max-height:calc(100vh - 100px)}" +
    "#bedi-panel.fit #bedi-body{flex:1 1 auto;overflow-y:auto;min-height:0}" +
    ...
    "#bedi-head{position:relative;z-index:6;padding:14px 12px 14px 18px;display:flex;align-items:center;gap:12px;background:#fff;border-bottom:1px solid #eef0f2}" +
    "#bedi-head .av{width:40px;height:40px;border-radius:50%;background:var(--cream, #F8FAF9);border:1px solid " + C.line + ";display:flex;align-items:center;justify-content:center;flex-shrink:0}" +
    "#bedi-head .nm{color:" + C.teal + ";font-weight:700;font-size:15px;line-height:1.2}" +
    "#bedi-head .st{color:#8a94a0;font-size:12px;display:flex;align-items:center;gap:6px;margin-top:2px}" +
    ...
    "#bedi-book{background:" + C.teal + ";border:none;color:#fff;cursor:pointer;width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-left:auto;flex-shrink:0}" +
    "#bedi-x{margin-left:8px;background:" + C.teal + ";border:none;color:#fff;cursor:pointer;font-size:23px;width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:background .12s,color .12s}" +
    "#bedi-x:hover{background:" + C.tealDeep + ";color:#fff}" +
    "#bedi-body{flex:1;overflow-y:auto;padding:20px 18px;display:flex;flex-direction:column;gap:16px;background:#fff;-webkit-overflow-scrolling:touch}" +
    ".bedi-row{display:flex;gap:8px;align-items:flex-end;animation:bediin .25s ease}" +
    ...
    ".bedi-b{width:fit-content;min-width:0;padding:10px 14px;font-size:14.5px;line-height:1.5;white-space:normal;overflow-wrap:break-word;word-break:normal;hyphens:none}" +
    ...
    ".bedi-team-l{font-size:11px;font-weight:700;color:" + C.coralDeep + ";margin:0 0 3px 2px}" +
    ".bedi-sys{align-self:center;font-size:11.5px;color:" + C.muted + ";background:#F0F4F3;padding:4px 11px;border-radius:20px}" +
    ".bedi-av{display:none}" +
    "#bedi-chips{padding:10px 14px 12px;display:flex;flex-wrap:wrap;gap:7px}#bedi-chips:empty{padding:0}" +
    ".bedi-chip{padding:8px 15px;font-size:13.5px;font-weight:500;border-radius:22px;background:#fff;color:" + C.teal + ";border:1.5px solid #FFE2D1;cursor:pointer}" +
    ".bedi-chip:hover{transform:translateY(-1px)}" +
    ".bedi-cta{display:inline-flex;align-items:center;gap:5px;padding:8px 14px;font-size:13.5px;font-weight:600;border-radius:20px;background:" + C.coral + ";color:#fff !important;border:1px solid " + C.coral + ";text-decoration:none;cursor:pointer;transition:background .12s}" +
    ".bedi-cta:hover{background:" + C.coralDeep + "}" +
    "#bedi-foot{padding:14px 16px;display:flex;align-items:center;gap:10px;border-top:1px solid #eef0f2;background:#fff}" +
    "#bedi-inwrap{flex:1;display:flex;align-items:center;gap:4px;background:#f4f6f8;border:1px solid #eef0f2;border-radius:24px;padding:0 6px 0 16px}" +
    "#bedi-in{flex:1;border:none;outline:none;padding:11px 0;font-size:16px;background:transparent;color:" + C.ink + "}" +
    ".bedi-ic{width:30px;height:30px;border:none;background:none;cursor:pointer;border-radius:50%;display:flex;align-items:center;justify-content:center}" +
    ".bedi-ic.on{background:#FFF3EB}" +
    "#bedi-send{width:42px;height:42px;border:none;border-radius:50%;cursor:pointer;background:" + C.teal + ";display:flex;align-items:center;justify-content:center;flex-shrink:0}" +
    ".bedi-dot{width:6px;height:6px;border-radius:50%;background:" + C.coral + ";display:inline-block;animation:bedid 1s infinite}" +
    "@keyframes bedid{0%,60%,100%{opacity:.4;transform:translateY(0)}30%{opacity:1;transform:translateY(-4px)}}" +
    "#bedi-form{position:absolute;left:0;right:0;top:68px;bottom:0;background:#fff;z-index:5;display:flex;flex-direction:column;animation:bediin .2s ease}" +
    "#bedi-intake{flex:1;min-height:0;display:flex;flex-direction:column;padding:0;width:100%;animation:bediin .2s ease}" +
    ".bedi-fh{padding:13px 16px;font-weight:700;font-size:14px;background:#fff;border-bottom:1px solid " + C.line + ";display:flex;align-items:center;color:" + C.ink + "}" +
    ".bedi-fh button{margin-left:auto;background:none;border:none;font-size:20px;cursor:pointer;color:" + C.ink + ";opacity:.6}" +
    ".bedi-fb{padding:16px;display:flex;flex-direction:column;gap:10px;overflow-y:auto}" +
    ".bedi-fi{padding:9px 2px;border:none;border-bottom:1.5px solid " + C.line + ";border-radius:0;outline:none;font-size:16px;background:transparent;color:" + C.ink + ";transition:border-color .16s}" +
    ".bedi-fi:focus{border-bottom-color:" + C.coral + "}" +
    ".bedi-fi::placeholder{color:#9aa3ad}" +
    ".bedi-seg{display:flex;gap:8px}.bedi-seg button{flex:1;padding:10px;border:1px solid " + C.line + ";background:#fff;border-radius:10px;cursor:pointer;font-size:13px;color:" + C.muted + "}.bedi-seg button.on{background:" + C.teal + ";color:#fff;border-color:" + C.teal + "}" +
    ".bedi-fbtn{padding:14px;border:none;border-radius:12px;background:" + C.coral + ";color:#fff;font-weight:700;cursor:pointer;font-size:15px;letter-spacing:.01em;transition:background .14s;margin-top:8px}" +
    ".bedi-fbtn:hover{background:" + C.coralDeep + "}" +
    ".bedi-fbtn:active{opacity:.92}" +
    ".bedi-fn{font-size:11px;color:" + C.muted + ";text-align:center}" +
    
    "#bedi-body.intake{padding:0}" +
    ".bedi-iw{position:relative;display:flex;flex-direction:column;gap:18px;padding:18px 20px 20px;min-height:0}" +
    "@media (max-height:720px){.bedi-iw{gap:12px;padding:14px 20px 16px}.bedi-fi{padding:8px 2px}.bedi-ita{min-height:44px}.bedi-it{font-size:17px}}" +
    ".bedi-it{font-weight:800;font-size:19px;color:" + C.ink + ";line-height:1.3;letter-spacing:-.01em}" +
    ".bedi-isub{font-size:13.5px;color:" + C.muted + ";margin:-8px 0 6px;line-height:1.45}" +
    ".bedi-ita{min-height:52px;resize:none;font-family:inherit;line-height:1.4}" +
    ".bedi-ierr{font-size:12px;color:#C0392B}.bedi-ierr:empty{display:none}" +
    
    /* Armor CSS Rules: Stop host theme CSS leakage */
    ...
    "#bediw #bedi-head .av svg{display:block!important;width:24px!important;height:24px!important;opacity:1!important;visibility:visible!important}" +
    "#bediw svg{vertical-align:middle;max-width:none!important}" +
    "#bediw .bedi-chip{background:#fff!important;color:" + C.teal + "!important;padding:6px 12px!important;font-size:13px!important;font-weight:500!important;border:1px solid " + C.line + "!important;border-radius:20px!important;width:auto!important;height:auto!important;box-shadow:none!important;text-align:center}" +
    "#bediw .bedi-cta{background:" + C.coral + "!important;color:#fff!important;padding:8px 14px!important;font-size:13.5px!important;font-weight:600!important;border:1px solid " + C.coral + "!important;border-radius:20px!important;width:auto!important;height:auto!important;box-shadow:none!important}" +
    "#bediw #bedi-send{width:42px!important;height:42px!important;padding:0!important;border:none!important;border-radius:50%!important;background:" + C.teal + "!important;box-shadow:none!important}" +
    "#bediw .bedi-ic{width:30px!important;height:30px!important;padding:0!important;border:none!important;border-radius:50%!important;background:none!important;box-shadow:none!important}" +
    "#bediw .bedi-ic.on{background:#FFF3EB!important}" +
    "#bediw #bedi-book{width:38px!important;height:38px!important;padding:0!important;border-radius:50%!important;background:" + C.teal + "!important;border:none!important;color:#fff!important;box-shadow:none!important;display:flex!important;align-items:center;justify-content:center;flex-shrink:0}" +
    "#bediw #bedi-x{width:38px!important;height:38px!important;padding:0!important;border-radius:50%!important;background:" + C.teal + "!important;border:none!important;color:#fff!important;box-shadow:none!important;display:flex!important;align-items:center;justify-content:center;flex-shrink:0}" +
    "#bediw #bedi-x{font-size:23px!important;margin-left:8px!important;color:#fff!important}" +
    "#bediw #bedi-book svg{width:21px!important;height:21px!important}" +
    "#bediw .bedi-fbtn{background:" + C.coral + "!important;color:#fff!important;padding:14px!important;border:none!important;border-radius:12px!important;width:100%!important;box-shadow:none!important;font-weight:700!important}" +
    "#bediw .bedi-seg button{background:#fff!important;color:" + C.muted + "!important;padding:10px!important;border:1px solid " + C.line + "!important;border-radius:10px!important;width:auto!important;box-shadow:none!important}" +
    "#bediw .bedi-seg button.on{background:" + C.teal + "!important;color:#fff!important;border-color:" + C.teal + "!important}";
  ...
  var micI = '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#596D6C" stroke-width="2"><rect x="9" y="2" width="6" height="11" rx="3"/><path d="M5 10a7 7 0 0014 0M12 17v4"/></svg>';
  var sendI = '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z"/></svg>';
  ...
  root.innerHTML =
    ...
    '<div id="bedi-panel">' +
      '<div id="bedi-head"><div class="av">' + medicalLogo + '</div><div><div class="nm">Dr. Rajeev Bedi\'s OPD</div>' +
        ...
        '<button id="bedi-x" aria-label="Close">&times;</button></div>' +
  ...
  // --- Intake Form: Capture Name/Mobile/Email up-front ---
  function showIntake() {
    body.innerHTML = "";
    $("bedi-foot").style.display = "none";   
    chipsEl.style.display = "none";
    body.className = "intake";
    var pn = $("bedi-panel"); if (pn) pn.className = (pn.className.replace(/\bfit\b/,"").trim() + " fit").trim();
  ...
  function submitIntake() {
    ...
    fetch(API + "/api/start", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: SID, name: name, phone: phone, email: email, message: msg }) })
      ...
      .then(function () {
        ...
        var pn2 = $("bedi-panel"); if (pn2) pn2.className = pn2.className.replace(/\bfit\b/,"").trim();
        body.className = "";
  ...
  // Auto-open / Nudge trigger on page load
  (function () {
    if (!intakeDone) {
      setTimeout(function () { if (!open) $("bedi-btn").click(); }, 800);
    } else {
